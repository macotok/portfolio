  ...
  [AC_DELETE_DATA](context, dataID) {
    switch (process.env.SWITCH_DATABASE) {
      ...
      case 'production': {
        ...
        const imagesRef = storageRef.child(`images/${FIREBASE_SKILL}/${payload.id}/${payload.image.name}`);
        imagesRef.delete()
          .then(() => {
            firestore.collection(FIREBASE_SKILL).doc(payload.id.toString(10)).delete();
          })
          .then(() => {
            dataList.splice(dataList.findIndex(data => data.id === dataID), 1);
            context.commit(MU_DELETE_DATA, dataList);
